...
* ~~Replay 功能。~~（[跳帧补偿会对控制流造成不可预测的影响-Issue-#5](https://github.com/Heaveeeen/JSTG/issues/5)）
...
本框架短期内不准备放出发行版本。我准备先拿这个框架做个游戏出来，借这个过程把该做的功能都做出来、打磨打磨，然后再从这个游戏中提取出框架来，把这个框架发布出去。*（就像海绵宝宝画圆，先画个头，擦出草图，再从草图里得到圆）*

## 版权声明

本框架准备采用 MIT 开源协议，但我暂且不想管这个。

本框架引用了 pixi.js 。部分素材借用自 Simple 的弹幕引擎模板（以250724版为主）和《东方Project》系列作品。LOGO 中使用了思源黑体作为字体。
